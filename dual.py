...
def crear_matrizDmax(matriz, variables, constraints, columns, rows, sign):
...
def minimizarDmax(matriz, variables, constraints, columns, rows, sign):
  '''Aqui se hacen las iteraciones para encontrar el optimo del modelo'''
  matriz,variables,rows,columns=crear_matrizDmax(matriz, variables, constraints, columns, rows, sign)
...
def crear_matrizDmin(matriz, variables, constraints, columns, rows, sign):
...
def maximizarDmin(matriz, variables, constraints, columns, rows, sign):
  '''Aqui se realizan las iteraciones para encontrar la maximización del modelo'''
  matriz,variables,rows,columns=crear_matrizDmin(matriz, variables, constraints, columns, rows, sign)
      ...
      for h in range(variables):
        for i in range(0,rows-1):
          if len(counx[h])>1:
            ...
            print(f"El valor de x{h+1} es ", Fraction(str(matriz[counx[h][0]][columns-1])).limit_denominator(100))
            break
      valmin = matriz[rows-1][columns-1]
      print(valmin)
      break
  return matriz, valmin, soluciones
